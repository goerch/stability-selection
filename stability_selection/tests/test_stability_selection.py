...
def _generate_dummy_regression_data(p=1000, n=1000, k=5,
    ...
    return X if not csr else sparse.csr_matrix(X), y, important_betas
...
def test_stability_selection_classification_with_selection():
    n, p, k = 1000, 1000, 5
...
def test_stability_selection_sparse_classification():
    n, p, k = 1000, 1000, 5

    X, y, important_betas = _generate_dummy_classification_data(n=n, k=k, csr=True)
    selector = StabilitySelection(lambda_grid=np.logspace(-5, -1, 25), verbose=1)
    selector.fit(X, y)
...
def test_stability_selection_sparse_regression():
    n, p, k = 500, 1000, 5

    X, y, important_betas = _generate_dummy_regression_data(n=n, k=k, csr=True)
